...
# --bind /data/ makes the /data/ directory accessible inside the container.
# bash -c runs the following command (specified by -c) in a shell (bash) inside the container.
...
# 2> ${output_dir}/${R1_file%_R1.fastq.gz}_hisat2_summary.log: Redirects HISAT2's error messages and alignment statistics
# to a log file. The log file contains information such as alignment rates, concordant/discordant alignments and multimapping reads.